...
SoftwareSerial mySerial(D5, D6); // RX, TX pin yng digunakandinode MCU

// You should get Auth Token in the Blynk App.
// Go to the Project Settings (nut icon).
char auth[] = "gVG-q7hJW6gL9FwC9BGe5FHODpgvnBXX"; // token blynk

// Your WiFi credentials.
// Set password to "" for open networks.
char ssid[] = "MorningStar"; // ssid wifi
char pass[] = "belidulubro"; //pasword
...
void myTimerEvent()
{
  ...
  Blynk.virtualWrite(V0, secondValue); //pin virtual di blynk yang dipakai
  Blynk.virtualWrite(V1, thirdValue);
  Blynk.virtualWrite(V2, fourthValue);
  Blynk.virtualWrite(V3, fifthValue);
}
void loop() {
  // put your main code here, to run repeatedly:
  Blynk.run();
  timer.run(); 
while(mySerial.available()>0){  //parsing data 
  c=mySerial.read();
  delay(10);
  data+=c;
  }
